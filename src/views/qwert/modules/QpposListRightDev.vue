  ...
  <page-layout :title="devName">
    <a-card :bordered="false">
      <detail-list title="环控点" >
        <detail-list-item :term="item.targetName" v-if="targets.length>0" v-for="(item,index) in targets" :key="index">{{ item.value }}</detail-list-item>
      </detail-list>
      <a-divider style="margin-bottom: 32px"/>
      <detail-list title="环控点 222" >
        <detail-list-item :term="item.targetName" v-if="targets2.length>0" v-for="(item,index) in targets2" :key="index">{{ item.value }}</detail-list-item>
      </detail-list>
      <a-divider style="margin-bottom: 32px"/>
